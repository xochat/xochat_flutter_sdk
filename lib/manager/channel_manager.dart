...
class XOChannelManager {
  XOChannelManager._privateConstructor();
  static final XOChannelManager _instance =
      XOChannelManager._privateConstructor();
  static XOChannelManager get shared => _instance;

  final List<XOChannel> _list = [];
  HashMap<String, Function(XOChannel)>? _refeshChannelMap;
  Function(String channelID, int channelType, Function(XOChannel) back)?
      _getChannelInfoBack;
  ...
  Future<XOChannel?> getChannel(String channelID, int channelType) async {
    XOChannel? channel;
    if (_list.isNotEmpty) {
  ...
  addOrUpdateChannels(List<XOChannel> list) {
    if (list.isEmpty) {
      return;
    }
    for (XOChannel liMChannel in list) {
      _updateChannel(liMChannel);
    }
    ChannelDB.shared.insertOrUpdateList(list);
  }

  addOrUpdateChannel(XOChannel channel) {
    _updateChannel(channel);
    _setRefresh(channel);
    ChannelDB.shared.saveOrUpdate(channel);
  }

  _updateChannel(XOChannel channel) {
    bool isAdd = true;
  ...
  _setRefresh(XOChannel liMChannel) {
  ...
  addOnRefreshListener(String key, Function(XOChannel) back) {
  ...
  addOnGetChannelListener(Function(String, int, Function(XOChannel)) back) {
