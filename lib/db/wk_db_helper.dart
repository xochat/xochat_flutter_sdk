...
class XODBHelper {
  XODBHelper._privateConstructor();
  static final XODBHelper _instance = XODBHelper._privateConstructor();
  static XODBHelper get shared => _instance;
  final dbVersion = 1;
  Database? _database;
  Future<bool> init() async {
    var databasesPath = await getDatabasesPath();
    String path = p.join(databasesPath, 'wk_${XOIM.shared.options.uid}.db');
  ...
  Future<bool> onUpgrade(Database db) async {
    String path = await rootBundle
        .loadString('packages/xochat_flutter_sdk/assets/sql.txt');
    ...
    for (int i = 0; i < names.length; i++) {
      ...
      if (version > maxVersion) {
        String sqlStr = await rootBundle
            .loadString('packages/xochat_flutter_sdk/assets/$version.sql');
