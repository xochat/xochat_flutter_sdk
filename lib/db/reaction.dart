...
class ReactionDB {
  ...
  Future<int> queryMaxSeqWithChannel(String channelID, int channelType) async {
    String sql =
        "select max(seq) seq from ${XODBConst.tableMessageReaction} where channel_id=? and channel_type=? limit 0, 1";
    int version = 0;
    ...
    if (list.isNotEmpty) {
      dynamic data = list[0];
      if (data != null) {
        version = XODBConst.readInt(data, 'seq');
      }
    }
    return version;
  }

  Future<List<XOMsgReaction>> queryWithMessageId(String messageId) async {
    List<XOMsgReaction> list = [];
    List<Map<String, Object?>> results = await XODBHelper.shared.getDB().query(
        XODBConst.tableMessageReaction,
        where: "message_id=? and is_deleted=0",
        whereArgs: [messageId],
        orderBy: "created_at desc");
    if (results.isNotEmpty) {
      for (Map<String, Object?> data in results) {
        list.add(XODBConst.serializeMsgReation(data));
      }
    }
    return list;
  }

  Future<List<XOMsgReaction>> queryWithMessageIds(
      List<String> messageIds) async {
    List<XOMsgReaction> list = [];
    List<Map<String, Object?>> results = await XODBHelper.shared.getDB().query(
        XODBConst.tableMessageReaction,
        where:
            "message_id in (${XODBConst.getPlaceholders(messageIds.length)}) and is_deleted=0",
  ...
  Future<bool> insertOrUpdateReactionList(List<XOMsgReaction> list) async {
  ...
  // insertOrUpdateReaction(XOMsgReaction reaction) async {
  ...
  // updateReaction(XOMsgReaction reaction) {
  //   var map = <String, Object>{};
  //   map['is_deleted'] = reaction.isDeleted;
  //   map['seq'] = reaction.seq;
  //   map['emoji'] = reaction.emoji;
  //   XODBHelper.shared.getDB().update(XODBConst.tableMessageReaction, map,
  //       where: "message_id=? and uid=?",
  //       whereArgs: [reaction.messageID, reaction.uid]);
  // }

  insertReaction(XOMsgReaction reaction) {
    XODBHelper.shared.getDB().insert(
        XODBConst.tableMessageReaction, getReactionMap(reaction),
  ...
  dynamic getReactionMap(XOMsgReaction reaction) {
    var map = <String, Object>{};
