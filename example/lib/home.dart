...
class ListViewShowDataState extends State<ListViewShowData> {
  ...
  _initListener() {
    ...
    XOIM.shared.connectionManager.addOnConnectionStatus('home',
      ...
      } else if (status == XOConnectStatus.kicked) {
    ...
    // 监听更新消息事件
    XOIM.shared.conversationManager.addOnRefreshMsgListener('chat_conversation',
        (msg, isEnd) async {
      bool isAdd = true;
    ...
    // 监听刷新channel资料事件
    XOIM.shared.channelManager.addOnRefreshListener("cover_chat", (channel) {
      for (var i = 0; i < msgList.length; i++) {
  ...
  void _getDataList() {
    Future<List<XOUIConversationMsg>> list =
        XOIM.shared.conversationManager.getAll();
    list.then((result) {
    ...
    if (uiConversation.channelName == '') {
      uiConversation.msg.getWkChannel().then((channel) {
        if (channel != null) {
          ...
          XOIM.shared.channelManager.fetchChannelInfo(
